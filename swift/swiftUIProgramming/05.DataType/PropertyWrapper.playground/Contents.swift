...
// propertyWrapper로 래핑하려는 프로퍼티에 초기값을 줄 수 없는 경우 (아래는 generic을 활용하여 값을 지정해 줄 수 없는 경우) parameterLabel로 wrappedValue를 지정해주면 된다.
@propertyWrapper
...
struct IntDemo {
    @MinMaxVal(min:10, max:200) var value: Int = 250
}
