...
class _MyWidgetState extends State<MyWidget> {
  // stream 상태를 관리하는 SreamController
  ...
  void _startSteam() {
    // ConnectionState.waiting 상태
    debugPrint('_startSteam');
    int count = 0;
    // StreamController를 통해 Stream을 생성
    // periodic: 2초마다 주기적으로 streamCongroller에 '데이터 1, 2, 3...' 문자열 데이터를 생성
    Timer.periodic(Duration(seconds: 2), (timer) {
      count++;
      _streamController.add(
        '데이터 $count',
      ); // 첫 번째 데이터 저장 후: ConnectionState.active 상태
      if (count == 10) {
        timer.cancel();
        _streamController.close(); // ConnectionState.done 상태
      }
    });
  }
